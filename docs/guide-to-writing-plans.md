...
Plan markdown files live in **`docs/plans/`**. Completed plans move to **`docs/plans/done/`** with **`DONE`** in the filename (e.g. **`docs/plans/done/0.1-DONE-base-plan.md`**). See **Done / archive** below. This document is intentionally **not** named `README.md` so it is not mistaken for a generic package readme.
...
When implemented: rename to include **`DONE`** (or **`REJECTED`**) after the version prefix and **move** into **`docs/plans/done/`** — e.g. **`docs/plans/done/0.5-DONE-dialog-terminal-workflows.md`**. Set **`Status: DONE`**. Keep open plans only under **`docs/plans/`**.
