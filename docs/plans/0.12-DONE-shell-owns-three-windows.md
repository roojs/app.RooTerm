...
**Status:** DONE — **superseded in part** by **0.15** / **0.15.1**: Connection is **not** a Shell role (GTK `present` / hide with main); Preferences moves to own process + ``ConfigUpdate`` (see `docs/plans/0.15.1-DONE-prefs-process-dbus-cleanup.md`). Shell slots remain **`main`** + **`preferences`** only. Title / FIFO identity stays **🚫**.
