...
**ℹ️** Builds on **`docs/plans/0.3-session-states-history-guake.md`** (sudo / LXC discover already land children in config). This plan replaces tree **`fill()`-as-refresh** and the leftover “host tree chrome” bullets that belonged here.
...
- **🔷** Then: tree **DnD**, top-bar **Add group**, LXC **console / attach** default.
...
## Follow-on chrome (after model)

- **🔷** Tree **drag-and-drop** — via gateway remove + append(new_parent, conn) (persist `parent_uuid`).
- **🔷** Top bar **Add group** — `append(null, group)` or `append(parent_group, group)`.
- **🔷** **TODO** Container management: context menu **console** / **attach**; `lxc_enter` default.
  - **🚫** `ConnDialog` / “Edit connection” on LXC rows (no real host fields; only enter mode matters).
  - Delete-only menu for now.
...
- **🔷** `⏳` Add group / console-attach via gateway.
- **🔷** `⏳` Tree **DnD** via gateway (not urgent — after model polish; before **0.3** Phase 16 VTE log if we do chrome next).
...
3. Phase 19 — add group / console-attach; DnD when wanted (not blocking VTE log)  
...
- LXC **discover parse** / sudo / key setup stay in **0.3**.
- **Host tree editing** (DnD, add group) and **console/attach** defaults live **here**.
- 0.3 Phase **13b** points at this plan.
