...
- **ℹ️** {@link MainWindow} always constructs with `decorated: false`, `resizable: false`, width/height as monitor **percents** (full-width drop-down geometry).
- **ℹ️** Extension docks on `map` / `shown` / Toggle finish (`dockWindow` → under panel, `make_above`, `stick`, full monitor width).
- **ℹ️** {@link Application} activate: create + `present` {@link MainWindow}, then `new GnomeShell(window).ensure()`.
- **ℹ️** {@link GnomeShell.ensure}: install/upgrade extension, enable when known; install-failure uses `Adw.AlertDialog` on the main window; “restart Shell” is mostly `GLib.warning` today (earlier modal / non-modal hint dialogs proved painful on an undecorated drop-down).
- **ℹ️** `--toggle-key` without a live window still builds a **throwaway** `Gtk.Window` as parent for binding work — same “holder” smell.
...
- **🔷** Ready: extension files present **and** Shell reports the extension **enabled** (`GetExtensionInfo` state == 1).
- **🔷** Not ready: missing, not known to Shell yet, or known but not enabled (typical Wayland after first install until session restart).
- **🔷** `GnomeShell.ensure` still installs / enables / explains restart; it no longer needs a fake parent.
...
- **🔷** `⏳` All ensure dialogs parent on the live {@link MainWindow}.
- **🔷** `⏳` Restore a clear restart hint when install/update left Shell unable to load the new code (not only `GLib.warning`).
- **🔷** `⏳` Remove throwaway `Gtk.Window` from `--toggle-key` path.

### Phase 3 — Extension: dock only when `DockMode`

- **🔷** `⏳` Add D-Bus property `DockMode`; app sets it from MainWindow mode.
- **🔷** `⏳` Extension skips under-panel remorph when `DockMode` is false; bump extension `version`.
- **🔷** `⏳` Underbar path still docks on Toggle / `Shown` as today when `DockMode` is true.

### Phase 4 — Remorph when Shell becomes ready

- **🔷** `⏳` On activate / toggle (and after setup dialog path): if Shell ready and window still normal → underbar + `DockMode` true + dock cue.
- **💩** `⏳` Manual check notes: fresh user dir → normal + `Adw.AlertDialog`; after logout/login → underbar; X11 `Alt+F2` `r` if applicable.
---
