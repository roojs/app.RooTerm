...
**Status:** ⏳ Phases 0–3 implemented (awaiting user verify on X11 + Wayland)
...
**ℹ️** Supersedes hunting in **`docs/bugs/2026-08-01-edit-connection-lost-on-hide.md`** (`FloatingCount`, width classify). Builds on three long-lived windows from MainWindow ctor (`connection_editor`, `preferences_editor`, main) plus {@link MainWindow.shell}.
...
- **🔷** `⏳` Show preferences/connection: centre once if needed, `make_above`, raise, activate; main yields above while either dialog is shown (Phase 2 polish).
- **🔷** `⏳` Confirm minimize Hide/Show leaves slots intact (no `unmanaged`) on **X11 and Wayland** — Phase 0 verify.
...
- **🔷** `⏳` Shell `Toggle('main')` Meta-alive check — user verify with Phase 2.
...
- **🔷** `⏳` User verify (X11 + Wayland): journal shows `stored role=` for **main**, **preferences**, **connection**; Hide does not `unmanaged cleared slot`.
...
- **🔷** `⏳` Verify hide/show + dialogs above main on **X11 and Wayland**.
- **🔷** `⏳` Mark bug **`docs/bugs/2026-08-01-edit-connection-lost-on-hide.md`** done after ✅ verify on both.
...
- **ℹ️** Phase 0 Hide uses **minimize**; revisit actor-hide / off-monitor if minimize is wrong on either session.
