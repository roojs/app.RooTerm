...
**ℹ️** Follows cwd / {@link TerminalStream} work in chat; Localhost path children from **`docs/plans/done/0.3-DONE-session-states-history-guake.md`**.
...
- **🔷** ✅ Avoid `HostPage` hunting / reconciling Localhost child `Connection` rows by parallel index with `terminals[]` (`sync_paths` style) — user: suspicious / bad design.
- **🔷** ✅ Remove {@link Terminal.update_cwd} — user: does not justify a helper; inline at call sites.
- **🔷** ✅ That tree child is **just the connection** — do not invent a second field (`path` / `tree_row`). Use {@link Terminal.connection} for the local shell’s `LOCAL_PATH` row.
...
- **🔷** ✅ No parallel-index `sync_paths` hunt
- **🔷** ✅ No `update_cwd`
- **🔷** ✅ No second field for the tree child — local tab’s `connection` is that row
- **💩** ✅ `cd` renames `term.connection.name`
- **💩** ✅ Close removes that tree row
- **💩** ✅ Click child focuses same VTE via `sessions`
- **💩** ✅ No `local_tab` / `HostPage.paths`
