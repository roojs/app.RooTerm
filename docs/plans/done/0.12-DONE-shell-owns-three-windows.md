...
**Status:** DONE — **superseded in part** by **0.15** / **0.15.1**: Connection is **not** a Shell role (GTK `present` / hide with main); Preferences moves to own process + ``ConfigUpdate`` (see `docs/plans/done/0.15.1-DONE-prefs-process-dbus-cleanup.md`). Shell slots remain **`main`** + **`preferences`** only. Title / FIFO identity stays **🚫**.
...
**ℹ️** Supersedes hunting in **`docs/bugs/done/2026-08-01-FIXED-edit-connection-lost-on-hide.md`** (`FloatingCount`, width classify). Historical three-window Register (`main` / `preferences` / `connection`) — connection Register dropped in 0.15 Phase 0.
